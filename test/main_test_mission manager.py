...
class Movimenti:
    ...
    def equazione(self, equazione, distanza_max, velocità, multithreading = None):
        ...
        global Kp
        global run_multithreading, inMain
    ...
    def seguiLinea(self, distanza, velocità, lato, multithreading = None):
        ...
        multithreading: definire la funzione che si vuole eseguire mentre il robot si sposta Es:
            ...
            vaiDrittoPID(1000, 80, multithreading=multithreading )
        '''
        global Kp, Ki, Kd, run_multithreading, runSmall, colorSensor, inMain # Dichiarazione delle variabili globali
...
def program_2():
    ...
    if inMain:
        return
    smallMotorD.run_for_degrees(45,-20) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    movement_motors.move(300, unit="degrees", steering=0, speed=-50) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    mv.oipocsoric(88,1)
    multi = avviaMotore(45,20,"D", spike) 
    mv.vaiDrittoPID(100,50,multi)
    if inMain:
        return
    movement_motors.move(200, unit="degrees", steering=0, speed=-50) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    time.sleep(0.1)
    smallMotorD.run_for_degrees(45,-20) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    mv.ciroscopio(5,-1)
    mv.vaiDrittoPID(150, 50)
    mv.ciroscopio(5,1)
    if inMain:
        return
    smallMotorD.run_for_degrees(50,100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    movement_motors.move(2500, unit="degrees", steering=-10, speed=-100) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo 
...
def program_3():
    ...
    motoreSinistro.run_for_degrees(630, 50) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    mv.ciroscopio(12, -1)
    mv.vaiDrittoPID(340, 50)
    if inMain:
        return
    #da questo momento in poi la variabile inMain non può mai diventare true perché sono tutte funzioni standard di spike, quindi è inutile fare il controllo
...
def program_4():
    ...
    if inMain:
        return
    movement_motors.move(450, unit="degrees", steering=0, speed=-100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
...
#2° linea fine
    ...
    if inMain:
        return
    smallMotorD.run_for_degrees(65, -80) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    mv.ciroscopio(45, -1)
    time.sleep(0.1)
    ...
    if inMain:
        return
    movement_motors.move(375, unit="degrees", steering=-45, speed=-50) #torna indietro - queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    ...
    if inMain:
        return
    smallMotorC.run_for_degrees(90, -70) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    movement_motors.move(500, unit="degrees", steering=0, speed=-15) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    time.sleep(0.2)
    smallMotorD.run_for_degrees(85, -80) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    movement_motors.move(300, unit="degrees", steering=0, speed=-100) #la variabile inMain non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
    mv.ciroscopio(30,-1)
    #mv.vaiDrittoPID(600,70)
    time.sleep(0.5)
    if inMain:
        return
    movement_motors.move(2500,unit="degrees",steering=10,speed=100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
...
def program_6():
    ...
    if inMain:
        return
    movement_motors.move(330, unit="degrees", steering=0, speed=-30) # torna indietro - queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    mv.ciroscopio(86, 1) # raccogli gamberi
    if inMain:
        return
    movement_motors.move(1300,unit="degrees",steering=-15,speed=100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    """"
    mv.vaiDrittoPID(50, 50) # partenza
    mv.ciroscopio(48, -1) # sinistra in area
...
def program_7():
    ...
    if inMain:
        return
    smallMotorC.run_for_degrees(850, 100) #abbassa gamberetti - queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    time.sleep(0.5)
    #da questo momento in poi la variabile inMain non può mai diventare true perché sono tutte funzioni standard di spike, quindi è inutile fare il controllo
...
def program_8():
    ...
    if inMain:
        return
    smallMotorD.run_for_degrees(-60,30) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
    time.sleep(1)
    mv.ciroscopio(52,-1)
    ...
    if inMain:
        return
    movement_motors.move(700, unit="degrees", steering=-40, speed=90) #queste funzioni della libreria spike non si fermano automaticamente se la variabile inMain = True, quindi ci vuole un controllo prima di eseguirle
...
while main: #ciclo main, il programma non essere mai
    inMain = True
    time.sleep(0.5)
    #selezione programma
    ...
    if spike.left_button.is_pressed():
        inMain = False
        time.sleep(0.50)
        while not inMain: #se la variabile inMain diventa True allora interrompi il ciclo di esecuzione
            if programma_selezionato == 1:
                spike.status_light.on("green")
                program_1()
                programma_selezionato = 2
                spike.light_matrix.write(programma_selezionato)
                #esce dal loop di esecuzione e ritorna al main
                inMain = True
                break 
            elif programma_selezionato == 2:
                spike.status_light.on("red")
