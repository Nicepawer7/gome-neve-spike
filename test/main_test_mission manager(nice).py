...
class Movimenti:
    ...
    def seguiLinea(self, distanza, velocità, lato, multithreading = None):
        ...
        multithreading: definire la funzione che si vuole eseguire mentre il robot si sposta Es:
            ...
            vaiDrittoPID(1000, 80, multithreading=multithreading )
        '''
        global Kp, Ki, Kd, run_multithreading, runSmall, colorSensor, stop # Dichiarazione delle variabili globali
    ...
    def motoriMovimento(self,distanza, sterzo, velocità):
        """,potrebbe essere necessario "spezzare" in più pacchetti il movimento,per evitare che venga
         interamente eseguito non permettendo lo stop,grazie mamma lego:
        gestione manuale della funzione di movimento del brick,così da poter gestire meglio
        la variabile di stop dell'esecuzione nelle sottofunzioni implementando semplificazioni nel
...
def race(program):
    """ sarebbe una scelta migliore usare la sintassi switch case introdotta in
    python 3.10, tuttavia il brick usa python 3.4, decisamente obsoleta
    in alternativa usiamo la sintassi if-else"""
    global stop
    if program == 1:
    ...
    if program == 3:
        ...
        motoreSinistro.run_for_degrees(630, 50) #la variabile stop non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
        mv.ciroscopio(12, -1)
    ...
    if program == 4:
        ...
        time.sleep(0.2)
        mv.motoriMovimento(450,0,-100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        return
    ...
    #2° linea fine
        ...
        time.sleep(0.2)
        #squalo
        smallMotorD.run_for_degrees(65, -80) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        mv.ciroscopio(45, -1)
        time.sleep(0.1)
        mv.vaiDrittoPID(100 , 75) #riposizionamento
        mv.motoriMovimento(375,-45,-50) #torna indietro - queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        ...
        #gabbie + rientro
        smallMotorC.run_for_degrees(90, -70) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        mv.motoriMovimento(500,0,-15) #la variabile stop non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
        time.sleep(0.2)
        smallMotorD.run_for_degrees(85, -80) #la variabile stop non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
        mv.motoriMovimento(300,0,-100) #la variabile stop non può mai diventare true perché la funzione è standard di spike, quindi è inutile fare il controllo
        mv.ciroscopio(30,-1)
        #mv.vaiDrittoPID(600,70)
        time.sleep(0.5)
        mv.motoriMovimento(2500,10,100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        return
    ...
    if program == 6:
        ...
        mv.motoriMovimento(330,0,-30) # torna indietro - queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        mv.ciroscopio(86, 1) # raccogli gamberi
        mv.motoriMovimento(1300,-15,100) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        """"
        mv.vaiDrittoPID(50, 50) # partenza
        mv.ciroscopio(48, -1) # sinistra in area
    ...
    if program == 7:
        ...
        smallMotorC.run_for_degrees(850, 100) #abbassa gamberetti - queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        time.sleep(0.5)
        #da questo momento in poi la variabile stop non può mai diventare true perché sono tutte funzioni standard di spike, quindi è inutile fare il controllo
    ...
    if program == 8:
        ...
        smallMotorD.run_for_degrees(-60,30) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
        time.sleep(1)
        mv.ciroscopio(52,-1)
        mv.vaiDrittoPID(520,50)
        mv.ciroscopio(38,1)
        mv.vaiDrittoPID(550,60)
        mv.motoriMovimento(700,-40,90) #queste funzioni della libreria spike non si fermano automaticamente se la variabile stop = True, quindi ci vuole un controllo prima di eseguirle
...
while main: #ciclo main, il programma non essere mai
    ...
    if spike.right_button.is_pressed():
        time.sleep(0.50)
        programma_selezionato += 1
        print("Missione selezionata:" + programma_selezionato)
        spike.light_matrix.write(programma_selezionato)
    ...
    if spike.left_button.is_pressed():
        ...
        spike.status_light.on(colors[programma_selezionato-1])
        print("AVVIO il programma: " + programma_selezionato)
